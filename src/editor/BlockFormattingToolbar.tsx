...
// ==========================================
// PORTAL-BASED TOOLTIP HELPER (Top-Level Stacking & No Clipping)
// ==========================================
function Tooltip({
  text,
  children,
  ...
  const handleMouseEnter = () => {
    if (triggerRef.current) {
      ...
      const minPadding = 65;
      const bubbleLeft = Math.max(minPadding, Math.min(btnCenterX, window.innerWidth - minPadding));
      const arrowOffset = Math.max(-40, Math.min(btnCenterX - bubbleLeft, 40));
    ...
    >
      ...
      {coords && typeof document !== 'undefined' && createPortal(
        ...
        >
          <div
            style={{
              transform: `translateX(${coords.arrowOffset}px) rotate(45deg)`,
            }}
            className="w-2 h-2 bg-slate-900 dark:bg-slate-800 -mb-1 shadow-xs border-t border-l border-slate-700/60"
          />
